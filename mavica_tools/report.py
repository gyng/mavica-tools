...
def _sector_map_html(sector_status: list[str]) -> str:
    ...
    for status, color in colors.items():
        char = chars[status]
        lines.append(
            f'<span style="color:{color}">{char}</span> {status} &nbsp; '
        )
    lines.append('</div>')
...
def _thumbnail_html(image_path: str, max_width: int = 200) -> str:
    ...
    try:
        ...
        return f'<img src="data:image/jpeg;base64,{b64}" alt="{escape(os.path.basename(image_path))}">'
    except Exception:
...
def generate_from_recovery_dir(recovery_dir: str, output_path: str = None, **kwargs) -> str:
    ...
    for subdir in ("extracted", "carved_images"):
        ...
        if os.path.isdir(d):
            image_dir = d
            for jpg in sorted(
                globmod.glob(os.path.join(d, "*.jpg"))
                + globmod.glob(os.path.join(d, "*.JPG"))
            ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate HTML recovery report"
    )
    parser.add_argument(
        "recovery_dir",
